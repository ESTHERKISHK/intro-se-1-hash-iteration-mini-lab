...
# Here's a small hash of favorite books, each keyed to a different person.
favorite_books = {
...
#    Add that info to the hash as a new key-value pair.
...
# 3. Now let's start iterating. For each person in the hash, print out a statement that says "___'s favorite book is ____" and fill in the blanks with that person's name and that person's favorite book.
...
# LEVEL 2: In the address_book.rb file, there's an array called "contacts" with 100 hashes inside of it. Each hash representes a person, and has that person's name, phone number, email, the company they work for, and their address.
#    Note: The quotes may make it look like the keys 'name' and 'phone' are strings, but the colon notation coverts them to symbols, so it will operate much like the books has we've been using so far.
#    That means you could print the 7th person's email using this line of code:
#    puts(contacts[6][:email]) # Uncomment this to see if it works. (Remember, array indexing starts at zero, so we use the number 6 to access the 7th name).
...
# 6. We're going to send a mass text to all our contacts, so we need everyone's phone number. Our texting program can do it in an instant if we put all the numbers in an array.
...
#    Create an array called ed_emails and put every .edu email address from our contacts into that array. Print the array when you're done to check your work.
